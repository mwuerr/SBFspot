...
int mqtt_publish(const Config *cfg, InverterData *inverters[])
{
	...
	for (uint32_t inv = 0; inverters[inv] != NULL && inv < MAX_INVERTERS; inv++)
	{
		...
		for (std::vector<std::string>::iterator it = items.begin(); it != items.end(); ++it)
		{
			...
			else if (key == "pac2")				FormatFloat(value, (float)inverters[inv]->Pac2, 0, prec, dp);
			else if (key == "pac3")				FormatFloat(value, (float)inverters[inv]->Pac3, 0, prec, dp);
			else if (key == "uac1")				FormatFloat(value, (float)inverters[inv]->Uac1 / 100, 0, prec, dp);
			else if (key == "uac2")				FormatFloat(value, (float)inverters[inv]->Uac2 / 100, 0, prec, dp);
			else if (key == "uac3")				FormatFloat(value, (float)inverters[inv]->Uac3 / 100, 0, prec, dp);
			else if (key == "iac1")				FormatFloat(value, (float)inverters[inv]->Iac1 / 1000, 0, prec, dp);
			else if (key == "iac2")				FormatFloat(value, (float)inverters[inv]->Iac2 / 1000, 0, prec, dp);
			else if (key == "iac3")				FormatFloat(value, (float)inverters[inv]->Iac3 / 1000, 0, prec, dp);
